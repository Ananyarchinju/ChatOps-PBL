...
export const startSlackBot = async () => {
  if (process.env.SLACK_BOT_TOKEN && process.env.SLACK_APP_TOKEN) {
    ...
    // Help command
    slackApp.command('/chatops-help', async ({ command, ack, respond }) => {
      await ack();
      const text = `Hi there! :wave: I am your ChatOps Assistant. Here's what I can do:
• \`/server-status\` - View server status metrics
• \`/chatops-build <job-name>\` - Trigger a Jenkins build
• \`/chatops-docker ps\` - List running Docker containers
• \`/chatops-docker restart <id>\` - Restart a Docker container`;
      await respond(text);
      await logSlackCommand(command.user_id, '/chatops-help', 'success', text);
    });

    // Status command
    slackApp.command('/server-status', async ({ command, ack, respond }) => {
      await ack();
      const blocks = [
        {
          "type": "section",
      ...
      await logSlackCommand(command.user_id, '/server-status', 'success', 'Status blocks sent');
    });

    // Build command
    slackApp.command('/chatops-build', async ({ command, ack, respond }) => {
      await ack();
      const jobName = command.text.trim();
      if (!jobName) {
        await respond(`:warning: Please provide a job name. Example: \`/chatops-build my-job\``);
        return;
      }
      ...
      try {
        ...
        await logSlackCommand(command.user_id, `/chatops-build ${jobName}`, 'success', result);
      } catch (error: any) {
        await respond(`:x: [ERROR] ${error.message}`);
        await logSlackCommand(command.user_id, `/chatops-build ${jobName}`, 'failed', error.message);
      }
    });

    // Docker command
    slackApp.command('/chatops-docker', async ({ command, ack, respond }) => {
      await ack();
      ...
      if (args === 'ps') {
        try {
          ...
          if (containers.length === 0) {
            ...
            await logSlackCommand(command.user_id, '/chatops-docker ps', 'success', 'No containers');
          } else {
            const blocks: any[] = [
              {
                "type": "section",
            ...
            await respond({ blocks });
            await logSlackCommand(command.user_id, '/chatops-docker ps', 'success', `${containers.length} containers found`);
          }
        } catch (error: any) {
          await respond(`:x: [ERROR] Failed to list Docker containers: ${error.message}`);
          await logSlackCommand(command.user_id, '/chatops-docker ps', 'failed', error.message);
        }
      } else if (args.startsWith('restart ')) {
        const containerId = args.replace('restart', '').trim();
        if (!containerId) {
          await respond(`:warning: Please provide a container ID. Example: \`/chatops-docker restart abc123def456\``);
          return;
        }
        ...
        try {
          ...
          await respond(`:white_check_mark: ${result}`);
          await logSlackCommand(command.user_id, `/chatops-docker restart ${containerId}`, 'success', result);
        } catch (error: any) {
          await respond(`:x: [ERROR] ${error.message}`);
          await logSlackCommand(command.user_id, `/chatops-docker restart ${containerId}`, 'failed', error.message);
        }
      } else {
        await respond(`:warning: Unknown docker command. I received: \`${command.text}\`. Try \`/chatops-docker ps\` or \`/chatops-docker restart <id>\``);
      }
    });
    ...
    slackApp.message(async ({ message, say }) => {
      ...
      if ('text' in message && message.text) {
        ...
        const handledCommands = ['chatops-help', 'server-status', 'chatops-build', 'chatops-docker'];
        if (!handledCommands.some(cmd => text.includes(cmd))) {
          await say(`I didn't quite catch that. Try typing \`/chatops-help\` to see what I can do! :robot_face:`);
        }
      }
    });
